...
export class MemberShipService {
  ...
  public async storeMembershipService(data) {
    return await firstValueFrom(
      this.httpClient
        .post(`${this.membershipUrl}`, this.adapterRequest(data), {
          headers: this.headers,
        })
        .pipe(map((response) => this.adapterResponseRequest(response)))
    );
  }
    ...
    return await firstValueFrom(
      this.httpClient
        .post(`${this.uploadFileUrl}`, formData, {
          headers: this.baseService.getAuthHeadersToFormData(),
        })
        .pipe(map((response) => this.adapterResponseRequest(response)))
    );
  }
